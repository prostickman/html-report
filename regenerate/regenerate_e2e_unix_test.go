...
func TestEndToEndHTMLGenerationFromSavedResult(t *testing.T) {
	...
	reportDir := filepath.Join("_testdata", "unix", "e2e")

	inputFile := filepath.Join("_testdata", "unix", "last_run_result")
	...
	for _, expectedFile := range expectedFiles {
		...
		wantContent, err := ioutil.ReadFile(filepath.Join("_testdata", "unix", "expectedE2E", "simpleSuiteRes", expectedFile))
		if err != nil {
